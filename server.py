...
def get_ranker():
    ...
    return jsonify({"ranker_contents": ranker_contents})
...
def set_ranker():
    ...
    ranker_url = (
        "https://lab.textdata.org/api/v4/projects/"
        + projectId
        + "/repository/files/search_eval.py?ref=master&private_token="
        + apiToken
    )
    ...
    with open(fullpath, "r") as fp:
        while num_lines < preview_length:
            ...
            for w in querytext.lower().split():
                ...
                (sub_str, cnt) = re.subn(
                    re.compile(r"\b{}\b".format(w)), format_string, formatted_line
                )
